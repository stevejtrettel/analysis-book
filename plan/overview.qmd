...
- **Definition**: f is *Darboux integrable* if upper integral = lower integral
...
**It's a vector space**:
- If f, g integrable and α, β ∈ ℝ, then αf + βg integrable
- ∫(αf + βg) = α∫f + β∫g
...
- χ_Q: discontinuous everywhere, not integrable
...
- ∫(Σaₙxⁿ) = Σaₙxⁿ⁺¹/(n+1)
- Used for ln(1+x), arctan, computing π
- You might expect limits generally behave this well

### The Convergence Problem

- **Failure Mode 1**: fₙ = n·χ_{[0,1/n]} → 0 but ∫fₙ = 1
  - Limit integrable, integrals don't converge correctly
- **Failure Mode 2**: fₙ = χ_{q₁,...,qₙ} ↗ χ_Q
  - Each fₙ integrable with ∫fₙ = 0
...
- Fourier series: need ∫ and Σ to interchange
- Parameter integrals: need ∫ and d/dt to interchange
- Power series were the exception; general limits fail

### A Sanity Check

- gₙ = tent functions ↗ χ_{(0,1)}
- ∫gₙ → 1 = ∫χ_{(0,1)}
- Limit interchange works here ✓

### The Key Observation

- In Failure Mode 2: ∫fₙ = 0 → 0, obvious candidate is 0
- Problem isn't wrong answer—it's no answer when there's an obvious candidate
- Could we extend by *defining* ∫χ_Q = 0?
...
- Lemma: If fₙ ∈ C[a,b] and fₙ ↘ 0 pointwise, then I(fₙ) → 0
- Key technical tool bridging pointwise convergence to integral convergence
- Proof uses compactness of [a,b]

### Upper Functions

- Definition: f ∈ L↑ if fₙ ∈ C[a,b] with fₙ ↗ f pointwise
- Examples: continuous functions, χ_{(a,b]}
- Define I⁺(f) = lim I(fₙ)

### Well-Definedness of I⁺

- Must show: different approximating sequences give same answer
- Key Lemma: If fₙ ↗ f and g ∈ C[a,b] with g ≤ f, then I(g) ≤ lim I(fₙ)
- Proof uses Dini's theorem on the gap hₙ = g − (fₙ ∧ g) ↘ 0
- Corollary: I⁺ well-defined

### Properties of I⁺

- Extension: agrees with I on C[a,b]
- Monotonicity: f ≤ g ⟹ I⁺(f) ≤ I⁺(g)
- Additive: I⁺(f + g) = I⁺(f) + I⁺(g)
- Positive homogeneity: c ≥ 0 ⟹ I⁺(cf) = cI⁺(f)
- MCT for L↑: fₙ ∈ L↑, fₙ ↗ f, sup I⁺(fₙ) < ∞ ⟹ f ∈ L↑ and I⁺(fₙ) → I⁺(f)

### The Linearity Problem

- L↑ is a cone, not a vector space
- Closed under addition and positive scalars
- Not closed under subtraction: if fₙ ↗ f, then −fₙ ↘ −f (wrong direction)
- We need subtraction for basic analysis

### Integrable Functions

- Definition: f ∈ L¹ if f = g − h for some g, h ∈ L↑ with I⁺(g), I⁺(h) < ∞
- Define I(f) = I⁺(g) − I⁺(h)
- Must verify well-defined: if g₁ − h₁ = g₂ − h₂, then I⁺(g₁) − I⁺(h₁) = I⁺(g₂) − I⁺(h₂)
- Proof: g₁ + h₂ = g₂ + h₁, both in L↑, apply additivity

### Properties of I on L¹
...
- Theorem (MCT for L¹): fₙ ∈ L¹, fₙ ↗ f, sup I(fₙ) < ∞ ⟹ f ∈ L¹ and I(fₙ) → I(f)
- Proof: WLOG fₙ ≥ 0 (subtract f₁), apply key lemma, use MCT for L↑

### What We've Built

- An integral on L¹ ⊃ C[a,b]
...
- Setup: F(t) = ∫ₐᵇ f(x,t) dx
- Theorem: If ∂f/∂t exists and |∂f/∂t| ≤ g ∈ L¹, then F'(t) = ∫ₐᵇ (∂f/∂t) dx
- Proof: Apply DCT to difference quotients

### Application: Integrating Series Term-by-Term

- Setup: Σfₙ where each fₙ ∈ L¹
- Theorem: If Σ∫|fₙ| < ∞, then ∫Σfₙ = Σ∫fₙ
- Proof: Partial sums dominated by Σ|fₙ|, apply DCT

### Completeness

- Theorem: If fₙ ∈ L¹ and Σ∫|fₙ₊₁ − fₙ| < ∞, then fₙ converges to some f ∈ L¹
- Equivalently: Cauchy sequences in the ∫|·| sense converge
- False for Darboux integrable functions
- "Completing the Integral"—we've filled the holes

### Series Closure

- Theorem: If fₙ ∈ L¹ and Σ∫|fₙ| < ∞, then Σfₙ ∈ L¹
- False for Darboux: fₙ = χ_{qₙ}, each integrable, sum is χ_Q
- L¹ closed under absolutely convergent series
...
- Definition: E is null if for every ε > 0, E covered by countably many intervals of total length < ε
- Equivalently: χ_E ∈ L¹ and I(χ_E) = 0
...
- Proof: cover nth set with intervals of total length ε/2ⁿ

### Almost Everywhere

- Definition: Property holds a.e. if it fails only on a null set
- "f = g a.e." means {x : f(x) ≠ g(x)} is null
- "fₙ → f a.e." means {x : fₙ(x) ↛ f(x)} is null

### The A.E. Modification Theorem

- Theorem: If f = g a.e. and f ∈ L¹, then g ∈ L¹ and I(f) = I(g)
- Striking: modify on infinitely many points, integral unchanged
- False for Darboux: f = 0, g = χ_Q, differ on null set, but χ_Q not Darboux integrable

### Measurable Sets

- Definition: E is measurable if χ_E ∈ L¹
...
- Definition: μ(E) = I(χ_E) for measurable E
- Properties: μ(E) ≥ 0, μ(∅) = 0, μ([a,b]) = b − a
- Countable additivity: μ(⊔Eₙ) = Σμ(Eₙ)
- Proof via DCT
...
- Proof sketch: MCT forces values on L↑, linearity forces values on L¹
- There was no choice—MCT determined everything

### Maximality

- Theorem: L¹ is the largest class where f integrable ⟹ |f| integrable
- "Absolute integrability" property
- Extensions beyond (like HK) lose this

### Connection to Derivatives

- Theorem: f ∈ L¹ iff f = F' a.e. for some absolutely continuous F
- FTC for Lebesgue: cleaner than Darboux version
- Preview of absolute continuity

### Completeness Revisited

- L¹ with d(f,g) = ∫|f − g| is complete
- In fact: L¹ is the completion of C[a,b] under this metric
- Every element of L¹ is a limit of continuous functions
- We added exactly the missing limits—nothing more

### A General Machine

- The construction used the length axiom μ([c,d]) = d − c only as input
- Positivity, linearity, Daniell's condition did the work
- Replace with μ((c,d]) = g(d) − g(c) for increasing right-continuous g
...
- Example: F(x) = x² sin(1/x²), differentiable everywhere, F' not Lebesgue integrable
- Tradeoff: HK has better FTC, but no Lᵖ theory
...
| 10.2 | Construction: L↑ for MCT, L¹ for linearity |
...
3. **Completeness**: L¹ is complete
4. **Series closure**: Absolutely convergent series stay in L¹
...
**Sets up for later chapters**:
- Metric spaces: L¹ as key completeness example
