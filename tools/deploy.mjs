 ...
 *   npm run deploy
 *
 * Refuses to run with uncommitted changes or unpushed commits, so the
 * deployed site can never drift from git history — the same guarantee
 * repo-connected CI gives, built on the one machine that has the whole
 * toolchain (TeX Live, headless Chromium). Each deploy is stamped with its
 * commit hash, visible in Netlify's deploy list.
 */
